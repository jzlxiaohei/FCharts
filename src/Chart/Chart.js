...
export default class Chart extends EventEmitter {
  ...
  _makeCanvasClear() {
    ...
    this._scaleRatio(this.drawCanvas, ratio)._scaleRatio(
      this.eventCanvas,
      ratio
    );
  }
  ...
  _addSeries(key, sOpt) {
    ...
    if (componentType === Constant.Component.AVG_LINE) {
      this.avgLineComponentList[key] = BuildDrawComponentByType(
        componentType,
        sOpt
      );
    } else if (componentType === Constant.Component.AVG_PRICE) {
      this.avgPriceComponentList[key] = BuildDrawComponentByType(
        componentType,
        sOpt
      );
    } else {
